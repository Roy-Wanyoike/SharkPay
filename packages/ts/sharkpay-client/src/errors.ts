/**
 * Typed error hierarchy for the SharkPay v1 API.
 *
 ...
 *
 * Client-side failures (request timeout, network error) surface as
 * {@link TimeoutError} / {@link NetworkError} with `status: 0`.
 */

import type { ErrorBody } from './types/common.js';
...
/**
 * Base class of every error thrown by this SDK. Carries the envelope's
 * `code`, the HTTP `status`, and optionally `request_id` / `details`.
 */
...
/**
 * Generic API error for non-2xx responses that do not map to a more
 * specific class: 404 `not_found`, 409 `state_conflict`, 422
 * business-rule rejections (`insufficient_funds`, `risk_blocked`,
 * `kyc_required`, ...), exhausted 5xx retries, and unparseable bodies
...
/** 401/403 — missing/invalid API key, or the key's scopes (or agent policy) deny the operation. */
export class AuthError extends SharkPayError {}

/** Construction options for {@link RateLimitError}. */
export interface RateLimitErrorInit extends SharkPayErrorInit {
  /**
   * Parsed `Retry-After` hint in milliseconds (numeric seconds or HTTP
   * date), when the server sent one.
   */
  retryAfterMs?: number;
}

/** 429 quota exceeded (per-key burst/sustained). Not auto-retried by the client. */
...
/** Client-side: the request exceeded `timeoutMs` and was aborted. `status: 0`. Not retried. */
export class TimeoutError extends SharkPayError {}

/** Client-side: the fetch failed before a response arrived (DNS/refused/TLS). `status: 0`. Retried like 5xx. */
export class NetworkError extends SharkPayError {}

/**
 * Parse the shared error envelope out of a JSON response body.
