...
/**
 * JPA mapping of the append-only {@code transfer_state_transitions} audit
 * table: one row per recorded state change (docs/DATA-MODEL.md §1). The
...
public class TransferTransitionEntity {
    ...
    @Column(name = "transfer_id", nullable = false, updatable = false, length = 40)
    public String transferId;

    @Column(name = "from_state", nullable = false, updatable = false, length = 12)
    public String fromState;

    @Column(name = "to_state", nullable = false, updatable = false, length = 12)
    ...
    public static TransferTransitionEntity of(Transfer transfer, StateTransition transition) {
        TransferTransitionEntity entity = new TransferTransitionEntity();
        entity.transferId = transfer.id();
