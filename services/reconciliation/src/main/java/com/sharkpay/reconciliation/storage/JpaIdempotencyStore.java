...
public final class JpaIdempotencyStore implements IdempotencyStore {
    ...
    @Override
    public java.util.Optional<StoredRequest> find(Scope scope, String idempotencyKey) {
        return jpa.findById(new IdempotencyKeyPk(scope.name(), idempotencyKey))
                .map(entity -> new StoredRequest(entity.getRequestFingerprint(), entity.getEntityId()));
    }
